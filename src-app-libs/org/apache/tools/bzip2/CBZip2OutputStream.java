 ...
 * 
 * <p>
 * For decompression <tt>CBZip2InputStream</tt> allocates less memory if the bzipped input is smaller than one block.
 * </p>
 * 
 * <p>
...
public class CBZip2OutputStream extends OutputStream implements BZip2Constants {
	 ...
	 * This constant is accessible by subclasses for historical purposes. If you don't know what it means then you don't need it.
	 * <p>
	 * If you are ever unlucky/improbable enough to get a stack overflow whilst sorting, increase the following constant and try again. In practice I have never
	 * seen the stack go above 27 elems, so the following limit seems very generous.
	 * </p>
	 */
	 ...
	 * 
	 * @return The blocksize, between {@link #MIN_BLOCKSIZE} and {@link #MAX_BLOCKSIZE} both inclusive. For a negative <tt>inputLength</tt> this method returns
	 *         <tt>MAX_BLOCKSIZE</tt> always.
	 * 
	 * @param inputLength The length of the data which will be compressed by <tt>CBZip2OutputStream</tt>.
	 */
	 ...
	 * 
	 * <p>
	 * <b>Attention: </b>The caller is resonsible to write the two BZip2 magic bytes <tt>"BZ"</tt> to the specified stream prior to calling this constructor.
	 * </p>
	 * 
	...
	private void endBlock() throws IOException {
		 ...
		 * value will not appear by chance in the compressed datastream. Worst-case probability of this event, for a 900k block, is about 2.0e-3 for 32 bits,
		 ...
		 * compression/ decompression donot rely on these statistical properties. They are only important when trying to recover blocks from damaged files.
		 */
		bsPutUByte( 0x31 );
		bsPutUByte( 0x41 );
		bsPutUByte( 0x59 );
	 ...
	 * 
	 * <p>
	 * This is the version using unrolled loops. Normally I never use such ones in Java code. The unrolling has shown a noticable performance improvement on JRE
	 * 1.4.2 (Linux i586 / HotSpot Client). Of course it depends on the JIT compiler of the vm.
	 * </p>
	 * 
	 * @param dataShadow _
	 * @param lo _
	...
	private void mainSort() {
		...
		for ( int i = 0; i <= 255; i++ ) {
			...
			// Step 1:
			/*
			 * Complete the big bucket [ss] by quicksorting any unsorted small buckets [ss, j]. Hopefully previous pointer-scanning phases have already
			...
			// Step 3:
			/*
			 * The ss big bucket is now done. Record this fact, and update the quadrant descriptors. Remember to update quadrants in the overshoot area too, if
			 * necessary. The "if (i < 255)" test merely skips this updating for the last bucket processed, since updating for the last bucket is pointless.
			 */
			bigDone[ ss ] = true;
