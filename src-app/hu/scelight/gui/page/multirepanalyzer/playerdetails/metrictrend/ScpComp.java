...
/**
 * Supply-capped % component.
 * 
 * @author Andras Belicza
 */
@SuppressWarnings( "serial" )
public class ScpComp extends GeneralTableStatsComp< Integer, Int > {
	
	/**
	 * Creates a new {@link ScpComp}.
	 * 
	 * @param displayName display name to be used when replays of stats rows are opened
	 * @param playerStats player stats to show Supply-capped % stats for
	 */
	public ScpComp( final String displayName, final PlayerStats playerStats ) {
		super( displayName );
		
		buildGui( playerStats, "<html>SC%<br>range</html>", Int.class );
	}
	
	@Override
	protected Collection< GeneralStats< Int > > calculateStats( final PlayerStats playerStats ) {
		final int scpGranularity = Env.APP_SETTINGS.get( Settings.MULTI_REP_SCP_GRANULARITY );
		
		// Calculate stats
		final Map< Integer, GeneralStats< Int > > generalStatsMap = new HashMap<>();
		for ( final Game g : playerStats.gameList ) {
			if ( !g.hasTrackerInfo ) // Supply-capped % is derived from tracker events
				continue;
			...
			for ( final Part refPart : g.parts ) { // Have to go through all participants as the zero-toon might be present multiple times
				...
				final int scp = ( (int) refPart.supplyCappedPercent / scpGranularity ) * scpGranularity;
				GeneralStats< Int > gs = generalStatsMap.get( scp );
				if ( gs == null )
					generalStatsMap.put( scp, gs = new GeneralStats< Int >( new Int( scp ) {
						final String stringValue = scp + "-" + ( scp + scpGranularity ) + " %";
